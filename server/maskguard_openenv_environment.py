...
class MaskguardOpenenvEnvironment(Environment):
    ...
    Example:
        >>> env = MaskguardOpenenvEnvironment()
        >>> obs = env.reset()
        >>> print(obs.echoed_message)  # "Maskguard Openenv environment ready!"
        >>>
        >>> obs = env.step(MaskguardOpenenvAction(message="Hello"))
    ...
    def reset(self) -> MaskguardOpenenvObservation:
        ...
        Returns:
            MaskguardOpenenvObservation with a ready message
        """
        self._state = State(episode_id=str(uuid4()), step_count=0)
        self._reset_count += 1

        return MaskguardOpenenvObservation(
            echoed_message="Maskguard Openenv environment ready!",
            message_length=0,
            done=False,
            reward=0.0,
        )
    ...
    def step(self, action: MaskguardOpenenvAction) -> MaskguardOpenenvObservation:  # type: ignore[override]
        ...
        Args:
            action: MaskguardOpenenvAction containing the message to echo

        Returns:
            MaskguardOpenenvObservation with the echoed message and its length
        """
